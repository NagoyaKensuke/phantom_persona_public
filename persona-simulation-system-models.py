...
複数のOpenAIモデルに対応し、モデルごとの出力の違いを検証可能
"""
...
@dataclass
class ModelConfig:
    """OpenAIモデルの設定を管理するデータクラス"""
    model_id: str  # モデルID（API呼び出し用）
    display_name: str  # 表示名（UI表示用）
...
class ModelManager:
    ...
    def _register_default_models(self):
        ...
        self.register_model(ModelConfig(
            ...
            description="最新のGPT-4.1モデル - 高度な推論能力と創造性"
        ))

        self.register_model(ModelConfig(
            model_id="gpt-4.1-mini-2025-04-14",
            display_name="GPT-4.1 Mini",
            description="GPT-4.1の軽量版 - バランスの取れた性能とコスト"
        ))

        self.register_model(ModelConfig(
            model_id="gpt-4.1-nano-2025-04-14",
            display_name="GPT-4.1 Nano",
            description="GPT-4.1の超軽量版 - 高速レスポンスと低コスト"
        ))
        ...
        self.register_model(ModelConfig(
            ...
            display_name="GPT-4o",
            description="GPT-4 Omni - マルチモーダル対応の高性能モデル"
        ))

        self.register_model(ModelConfig(
            model_id="gpt-4o-mini-2024-07-18",
            display_name="GPT-4o Mini",
            description="GPT-4o Miniバージョン - 効率的で高速"
        ))
        ...
        self.register_model(ModelConfig(
            ...
            display_name="o3",
            description="o3推論モデル - 複雑な問題解決に特化"
        ))

        self.register_model(ModelConfig(
            model_id="o3-mini-2025-01-31",
            display_name="o3 Mini",
            description="o3の軽量版 - 推論能力とコストのバランス"
        ))
        ...
        self.register_model(ModelConfig(
            ...
            description="o4 Miniモデル - 次世代の効率的な推論"
        ))
...
class PersonaSystemUI:
    ...
    def setup_model_client(self, model_id: str):
        """
        モデルクライアントのセットアップ
    ...
    def display_models(self):
        ...
        for i, (model_id, display_name, description) in enumerate(models, 1):
    ...
    def select_model(self) -> str:
        ...
        while True:
            try:
                choice = int(input("使用するモデルを選択してください (番号): "))
                if 1 <= choice <= len(models):
                    selected_model = models[choice - 1][0]
                    print(f"\n選択されたモデル: {models[choice - 1][1]}")
    ...
    def display_current_model(self):
        ...
        if self.current_model:
            config = self.model_manager.get_model_config(self.current_model)
            print(f"\n現在のモデル: {config.display_name} ({config.model_id})")
...
async def main():
    ...
    # モデルの選択
    print("\nまず、使用するAIモデルを選択してください。")
    selected_model = ui.select_model()
    ui.setup_model_client(selected_model)

    while True:
        print("\n" + "=" * 60)
        print("メインメニュー")
        print(f"現在のモデル: {ui.model_manager.get_model_config(selected_model).display_name}")
        print("=" * 60)
        print("1. 単一ペルソナとの会話")
